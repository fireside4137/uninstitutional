...
> **Last Updated:** Session 12 (Privacy Enhancements — Policy, Delete Account, Export Data)
...
| Session 11 | User Profile Picture & Exam Card Watermarks | Added optional profile picture upload/change/remove capability in the profile/settings modal. Compressed image files (<1.5MB, JPG/PNG/WEBP) client-side using canvas scaling and JPEG quality adjustments, and saved base64 URLs in the `User.image` database field with bilingual validation alerts. Refined landing page exam cards (UKPSC and UKSSC) with theme-specific background watermark SVGs (an official building with columns, dome, and Himalayan peaks for UKPSC, and clipboard checks style document lines for UKSSC) with 5% opacity. Verified lint and build tests. |
| Session 12 | Privacy Enhancements (Policy, Delete Account, Export Data) | Created public bilingual `/privacy` page covering 11 policy sections (data collection, password security, learning data, access control, security logging, payment data, user rights, infrastructure, policy changes). Added `isDeleted`/`deletedAt` fields to User model and 4 new `SecurityEventType` enum values (`DATA_EXPORT_REQUESTED`, `ACCOUNT_DELETION_REQUESTED`, `ACCOUNT_DELETED`, `ACCOUNT_DELETION_FAILED`). Built `/api/user/export-data` (GET, authenticated JSON download of all user-owned data excluding secrets) and `/api/user/delete-account` (POST with "DELETE" text confirmation, anonymization strategy: name→"Deleted User", email→`deleted_<id>@deleted.local`, phone/image nulled, passwordHash replaced, isVerified=false, isDeleted=true). Added explicit `isDeleted` check in `auth.ts` authorize() as defense-in-depth. Added "Privacy & Data" section to dashboard settings modal with View Privacy Policy, Export My Data, and Delete Account buttons. Added Delete Confirmation modal (z-60) with red danger styling and "DELETE" text input guard. Added Privacy Policy link to homepage footer. All bilingual (EN/HI). |
